...
%
% function [xk, fk, gradfk_norm, xseq, btseq, pcg_iters_seq] = modified_newton_method(x0, f, gradf, hessf, kmax, tolgrad, c1, btmax, n, delta, rho, h, flag_h)
...
% failure = flag which indicates if there is stagnation during backtracking
...
while k < kmax && gradfk_norm > tolgrad
    ...
    while bt < btmax && fnew > farmijo(fk, alpha, c1_gradfk_pk)
        ...
        if abs(alpha) <= 10^(-6) % check for stagnation
            failure = true;
            break
        end
        xnew = xk + alpha * pk;
